...
void distance_vec(float *x, float *y, float *z, float *d, int n)
{
    ...
    for (int i = 0; i < k; ++i) {
        dd[i] = _mm_mul_ps (xx[i], xx[i]);
        tmp   = _mm_mul_ps (yy[i], yy[i]);
        dd[i] = _mm_add_ps (tmp, dd[i]);
        tmp   = _mm_mul_ps (zz[i], zz[i]);
        dd[i] = _mm_add_ps (tmp, dd[i]);
        dd[i] = _mm_sqrt_ps (dd[i]);
    }

    for (int i = 0; i < n % 4; ++i) {
        d[k * 4 + i] = x[k * 4 + i] * x[k * 4 + i] +
                       y[k * 4 + i] * y[k * 4 + i] +
                       z[k * 4 + i] * z[k * 4 + i];
        d[k * 4 + i] = sqrtf (d[k * 4 + i]);
    }
}
